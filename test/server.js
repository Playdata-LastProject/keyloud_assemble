...
// 선택한 폴더의 파일을 가져오기 위한 새로운 라우트 추가
app.get("/files_in_folder", async (req, res) => {
  try {
    const folderName = req.query.folderName;

    const collection = conn.db.collection("test");

    // 지정된 폴더에 있는 파일을 가져옵니다.
    const filesInFolder = await collection
      .find({ selectedFolder: folderName }, { filename: 1, uploadDate: 1 })
      .toArray();

    res.json(filesInFolder);
  } catch (error) {
    console.error("폴더 내 파일을 가져오는 중 오류 발생:", error);
